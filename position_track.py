...
for m in range(0,count):
    ...
    globals()['caseid'+str(m+1)]=0
    globals()['caseid'+str(m+1)]=input()
    globals()['caseid'+str(m+1)]='d'+globals()['caseid'+str(m+1)].zfill(3)
    datadir="../run/"+globals()['caseid'+str(m+1)]+"/data/"
    ...
    globals()['cx'+str(m+1)]=np.zeros(nd)
    globals()['cy'+str(m+1)]=np.zeros(nd)
    ...
    if (nd != 0):
        ...
        for n in range(n0,nd):
            ...
            globals()['cy'+str(m+1)][n]=centroids[1,0]
            globals()['cx'+str(m+1)][n]=centroids[1,1]
...
for m in range(0,count):
    ax1.plot(globals()['cy'+str(m+1)]*dy*1.e-8,(xmin-rsun+globals()['cx'+str(m+1)]*dx)*1.e-8,marker='D',label=globals()['caseid'+str(m+1)])
    #plt.legend()
"""
...
ax1.set_ylim((xmin-rsun)*1.e-8,(xmax-rsun)*1.e-8)
ax1.set_xlabel('y [Mm]')
ax1.set_ylabel('x [Mm]')
plt.legend()
plt.tight_layout()